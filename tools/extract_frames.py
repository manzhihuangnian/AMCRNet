...
def main(video_file, frame_rate=30, clip_start=900, clip_end=1801):
    ...
    try:
        frame_path = os.path.join(frame_root, 'img_%05d.jpg')
        ffmpeg_command = 'ffmpeg -ss {} -i {} -qscale:v 1 -t 901 -vf "scale=-1:340,fps={}" {} -loglevel error < /dev/null'.format(
            clip_start,
            video_path,
            frame_rate,
            frame_path,
