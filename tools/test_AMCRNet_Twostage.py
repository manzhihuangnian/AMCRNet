...
from mmaction.models import build_model,build_acdetector
from mmaction.utils import (build_ddp, build_dp, default_device,
                            register_module_hooks, setup_multi_processes)
import shutil
from mmaction.apis import multi_gpu_test_twostage, single_gpu_test_twostage
...
def inference_pytorch(args, cfg, distributed, data_loader):
    ...
    if not distributed:
        ...
        outputs = single_gpu_test_twostage(model, data_loader,LFB_cfg=cfg.model.head.Dynamic_LFB)
    else:
        model = build_ddp(
            model,
            default_device,
        ...
        outputs = multi_gpu_test_twostage(model, data_loader, args.tmpdir,gpu_collect=args.gpu_collect,
                                          LFB_cfg=cfg.model.head.Dynamic_LFB
                                 )
...
def main():
    ...
    # dataset.evaluate(results="./results.csv")
    ...
    if not os.path.exists(output_config['out']):
        if args.tensorrt:
    ...
    if rank == 0:
        ...
        if eval_config:
            if outputs:
                ...
                dir_path=os.path.dirname(output_config["out"])
                new_path=os.path.join(dir_path,"result_temporary",os.path.basename(output_config["out"]))
                if not os.path.exists(os.path.join(dir_path,"result_temporary")):
                    os.mkdir(os.path.join(dir_path,"result_temporary"))
                shutil.copy(output_config["out"],new_path)
                eval_res = dataset.evaluate(new_path, **eval_config)
